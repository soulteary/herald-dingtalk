...
生产上线、Kubernetes、探针、扩缩容和故障分流另见 [OPERATIONS.md](OPERATIONS.md)。
...
```bash
go build -ldflags "-w -s -X 'github.com/soulteary/version-kit/v4.Version=1.0.0' -X 'github.com/soulteary/version-kit/v4.Commit=$(git rev-parse --short HEAD)'" -o herald-dingtalk .
```
...
- **优雅关闭**：进程监听 `SIGINT`/`SIGTERM`，停止接收新连接并等待在途请求，超时为 35 秒，高于请求允许的 30 秒最大超时；监听失败会回传主协程并以非零状态退出。
