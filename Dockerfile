...
RUN BUILD_DATE=${BUILD_DATE:-$(date +%FT%T%z)} && \
    go build -ldflags "-w -s -X 'github.com/soulteary/version-kit/v4.Version=$VERSION' -X 'github.com/soulteary/version-kit/v4.Commit=$COMMIT' -X 'github.com/soulteary/version-kit/v4.BuildDate=$BUILD_DATE'" -o herald-dingtalk .
